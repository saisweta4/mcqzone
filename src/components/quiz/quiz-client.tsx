    ...
    <div className="flex flex-col h-screen bg-[#f8fafc] overflow-hidden">
      ...
      <header className="h-16 bg-white border-b flex items-center justify-between px-6 shrink-0 z-10">
        <div className="flex items-center gap-4">
          <Link href={`/exams`} className="text-gray-900 hover:bg-gray-100 p-2 rounded-full transition-colors">
            <ChevronLeft className="h-5 w-5" />
          </Link>
          <h1 className="text-base font-bold text-gray-900">
            {examName}: {subjectName} Practice
          </h1>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="hidden md:flex flex-col w-72 bg-white border-r shrink-0">
          <div className="p-4 border-b bg-gray-50/50">
            ...
            <div className="flex gap-3 text-xs text-gray-600 font-medium">
              <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-blue-600"></div> Current</div>
          ...
          <div className="p-4 grid grid-cols-5 gap-2 overflow-y-auto">
            {questions.map((q, idx) => {
              ...
              if (isCurrent) {
                btnStyle = "border-blue-600 bg-blue-600 text-white font-bold shadow-md"; // Active question
              } else if (isAnswered) {
        ...
        <main className="flex-1 flex flex-col overflow-y-auto relative">
          <div className="max-w-3xl w-full mx-auto p-6 md:p-10 flex-1 flex flex-col">
              ...
              {question.options.map((option) => {
                ...
                return (
                  <div 
                    ...
                    className={`flex items-center gap-4 p-5 rounded-xl border-2 transition-all ${cardStyle} ${!hasAnsweredCurrent && 'hover:shadow-sm'}`}
                  >
            ...
            <div className="mt-auto pt-6 flex items-center justify-between gap-4">
              <button 
                ...
                className="flex items-center gap-2 px-6 py-3 rounded-lg border border-gray-200 text-sm font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
              ...
              <button 
                ...
                className="flex items-center gap-2 px-6 py-3 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
              >
        ...
        {hasAnsweredCurrent && (
          <aside className="w-[400px] bg-[#f8fafc] border-l flex flex-col shrink-0 animate-in slide-in-from-right-8 duration-300 shadow-xl">
            ...
            <div className="p-6 border-t bg-white shrink-0">
              <button className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-white border border-gray-200 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-50 transition-colors shadow-sm">
                <HelpCircle className="h-4 w-4 text-gray-400" /> Ask AI a Follow-up
              </button>
            </div>
          </aside>
        )}
